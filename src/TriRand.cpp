 ...
 */
void TriRand::activateSites() {
    LatticeRandom::activateSites();
    generateBonds();
}

void TriRand::generateBonds() {


    // Start by making a triangular lattice
	size_t x_val, y_val;
    size_t i, k;
    for(i=0; i<num_sites; ++i) {
        ...
        k = ( x_val + 1 )%length + (( y_val - 1 + length )%length) * length;
        neighbor[i][2] = k;
        neighbor[k][5] = i;
    }
